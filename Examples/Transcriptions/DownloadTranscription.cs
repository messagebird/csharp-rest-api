...
{
    internal class DownloadTranscription
    {
        ...
        internal static void Main(string[] args)
        {
            ...
            try
            {
                using (var transcriptionDataStream = client.DownloadTranscription("CALL ID", "LEG ID", "RECORDING ID", "TRANSCRIPTION ID"))
                { 
                    using (var fileStream = File.OpenWrite(@"PATH TO FILE ON YOUR LOCAL MACHINE"))
                    {
                        transcriptionDataStream.CopyTo(fileStream);
                    }
                }
            }
